...
It is reflexive, because every integer is at most itself; it is antisymmetric, because if $a \le b$, then $b \le a$ only if $a=b$; it is transitive, because if $a \le b$ and $b \le c$, then $a \le c$.
...
This paper proves that exactly one of (a) $x \prec y$, (b) $x=y$, (c) $y\prec x$, or (d) $\incomp(x,y)$ holds for any given partial order $\preceq$ and its reflexive reduction $\prec$.
...
This paper uses the calculational logic system $\mathcal{E}$ to prove the quadrotomy---elements $x$ and $y$ of a partial order satisfy exactly one of (a) $x \prec y$, (b) $x=y$, (c) $y\prec x$, or (d) $\incomp(x,y)$, where incomp is defined as $\incomp(x,y) \equivs \lnot (x\preceq y) \land \lnot (y\preceq x)$ and $\prec$ is the reflexive reduction of $\preceq$.
The quadrotomy is a generalization of the trichotomy---elements $x$ and $y$ of a total order satisfy exactly one of (a) $x < y$, (b) $x=y$, or (c) $y < x$, where < is any total order.
...
The trychotomy of the at most relation is proved in the solutions manual of LADM using the properties of integers.

...
To prove that two elements $a$ and $b$ are related by exactly one of (a) $x \prec y$, (b) $x=y$, (c) $y\prec x$, or (d) $\incomp(x,y)$, we prove the four-part theorem:
...
The reasoning behind the choice of these theorems is discussed further in Subsection \ref{Gries}.

To prove this four-part theorem, we first prove several lemmas.
The first lemma, $b \prec c \lors b = c \equivss b \preceq c$, where $\prec$ is the reflexive reduction of $\preceq$, states that comparing two elements under a partial order is equivalent to comparing them under a strict order or equality.
...
The second lemma, $(p \lors q) \lands \lnot (q \lors r) \equivss p \lands \lnot q \lands \lnot r$, is based solely on propositional calculus.
...
The third lemma, $(p \impl q) \impl (p \lands q \equivs p)$, is an implication.
...
The fourth lemma, $\rho$ is irreflexive $\impl (b \rho c \impl \lnot (b = c))$, follows closely from the definition of irreflexivity, and is used much like Lemma 3: when the antecedent is true, the consequent can be used as a theorem.

The fifth lemma, Lemma 5: $\rho$ is antisymmetric $\lands$ $\rho$ is reflexive $\impl (b \rho c \lands c \rho b \equivs b = c)$, is an important property of partial orders: if $b \preceq c$ and $c \preceq b$, then $b = c$, because no element can both precede and follow any other element.
Like Lemmas 3 and 4, when the conjuncts of the antecedent of this theorem are true, the consequent can be used as a theorem.\\
...
It will be noticed that these theorems closely mirror the four theorems of Quadrotomy, but with the disjunct $\incomp(b,c)$ removed.
...
Lemma 7 follows from this table—in particular, from the equivalence of the second and third elements of the third column.
...
Lemma 7 is stated without proof, since as mentioned the proof is tedious.\\

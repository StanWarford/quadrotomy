...
In the solutions manual for LADM, Gries and Schneider prove using the properties of integers a trychotomy for $\le$, which is an example of a total order.
...
\]
is an important property of partial orders: if $b \preceq c$ and $c \preceq b$, then $b = c$, because no element can both precede and follow any other element.

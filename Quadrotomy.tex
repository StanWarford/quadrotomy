...
In general, elements$x$ and $y$ of a partial order satisfy exactly one of (a) $x \prec y$, (b) $x=y$, (c) $y\prec x$, or (d) $\incomp(x,y)$, where incomp is defined as $\incomp(x,y) \equivs \lnot (x\preceq y) \land \lnot (y\preceq x)$ and $\prec$ is the reflexive reduction of $\preceq$.
This is a generalization of the trichotomy of the at most relation $\le$, which states that integers $a$ and $b$ satisfy exactly one of (a) $a<b$, (b) $a=b$, or (c) $a>b$.

This paper demonstrates the power of $\mathcal{E}$ for proofs in discrete mathematics, in spite of its lack of wide adoption.
